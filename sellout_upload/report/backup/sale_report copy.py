...
class SellingSaleReport(models.Model):
    _name = "sellout.sale.report"
    ...
    country_id = fields.Many2one(
        comodel_name='res.country', string="Wilayah: Negara", readonly=True)
    industry_id = fields.Many2one(
        comodel_name='res.partner.industry', string="Kategory: Level2", readonly=True)
    partner_zip = fields.Char(string="Customer ZIP", readonly=True)
    state_id = fields.Many2one(comodel_name='res.country.state', string="Wilayah: Propinsi", readonly=True)
    district_id = fields.Many2one(comodel_name='res.district', string="Wilayah: Kota/Kab", readonly=True)
    area    = fields.Char(string="Area Marketing", readonly=True)
    ...
    partner_category1 = fields.Many2one('res.partner.categorylevel1','Kategory: Level0', readonly=True)
    partner_category2 = fields.Many2one('res.partner.categorylevel2','Kategory: Level1', readonly=True)

    hna = fields.Float(string="Hna", readonly=True)
    ...
    def _select_sale(self):
        select_ = f"""
            ...
            */
            ...
            d1.area AS area,
            lvl1.parent_id as partner_category1,
            lvl2.parent_id as partner_category2,
    ...
    def _from_sale(self):
        ...
        return f"""
            ...
            LEFT JOIN res_district d1 ON d1.id = partner.district_id             
            LEFT JOIN res_partner_industry lvl2 ON lvl2.id = partner.industry_id 
            LEFT JOIN res_partner_categorylevel2 lvl1 ON lvl2.parent_id = lvl1.id 
            LEFT JOIN res_partner_categorylevel1 lvl0 ON lvl1.parent_id = lvl0.id 
