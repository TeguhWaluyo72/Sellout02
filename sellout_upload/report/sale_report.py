...
class SaleReport(models.Model):
    ...
    industry_id = fields.Many2one(
        comodel_name='res.partner.industry', string="Jenis Pelanggan", readonly=True)
    partner_zip = fields.Char(string="Customer ZIP", readonly=True)
    state_id = fields.Many2one(comodel_name='res.country.state', string="Wilayah: Provinsi", readonly=True)
    district_id = fields.Many2one(comodel_name='res.district', string="Wilayah: Kota", readonly=True)
    ...
    def _select_sale(self):
        select_ = f"""
            ...
            END AS price_total,
            CASE WHEN l.product_id IS NOT NULL THEN SUM(l.x_hnatotal
                / {self._case_value_or_one('s.currency_rate')}
                * {self._case_value_or_one('account_currency_table.rate')}
                ) ELSE 0
            END AS hna,
