...
class SaleReport(models.Model):
    ...
    salesman_id = fields.Many2one(comodel_name='sellout.salesman',string="Salesman",readonly=True)
    ...
    industry_id = fields.Many2one(
        comodel_name='res.partner.industry', string="Kategory Pelanggan", readonly=True)
    partner_category2 = fields.Many2one(
        comodel_name="res.partner.categorylevel2", string="Middle Kategory Pelanggan", readonly=True )
    partner_category1 = fields.Many2one(
        comodel_name="res.partner.categorylevel1", string="Top Kategory Pelanggan", readonly=True )

    district_id = fields.Many2one(
        comodel_name='res.district', string="Kota/Kabupaten" , readonly=True)        
    partner_zip = fields.Char(string="Customer ZIP", readonly=True)
    state_id = fields.Many2one(comodel_name='res.country.state', string="Provinsi", readonly=True)
    ...
    ## Additional 
    rp_hna   = fields.Float(string="HNA", readonly=True)
    ...
    def _select_sale(self):
        select_ = f"""
            ...
            CASE WHEN l.product_id IS NOT NULL THEN SUM(l.rp_hna
                / {self._case_value_or_one('s.currency_rate')}
                * {self._case_value_or_one('account_currency_table.rate')}
                ) ELSE 0
            END AS rp_hna,
            ...
            res_partner_categorylevel2.id AS partner_category2,
            res_partner_categorylevel1.id AS partner_category1,
            res_district.state_id as state_id,
    ...
    def _from_sale(self):
        ...
        return f"""
            ...
            LEFT JOIN res_partner_industry ON partner.industry_id   = res_partner_industry.id 
            LEFT JOIN res_partner_categorylevel2 ON res_partner_industry.parent_id = res_partner_categorylevel2.id 
            LEFT JOIN res_partner_categorylevel1 ON res_partner_categorylevel2.parent_id = res_partner_categorylevel1.id 
            LEFT JOIN res_district ON partner.district_id = res_district.id 
            LEFT JOIN res_country_state ON res_country_state.id = res_district.state_id 
    ...
    def _group_by_sale(self):
        return """
            ...
            partner.country_id,
            partner.industry_id,
            res_partner_categorylevel2.id,
            res_partner_categorylevel1.id,
            res_district.state_id,
            partner.district_id,
            partner.zip,
            l.is_downpayment,
            l.discount,
